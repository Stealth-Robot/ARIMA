...
def themes_list():
    ...
    all_themes = Theme.query.order_by(Theme.id).all()
    # Move current user's personal theme to top
    themes = sorted(all_themes, key=lambda t: (0 if t.user_id == current_user.id else 1, t.id))
    theme_data = []
    for t in themes:
        if t.name:
            display_name = t.name
