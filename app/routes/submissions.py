...
def submissions_page():
    ...
    for sub in submissions:
        sub._entity_name = _entity_name(sub, cache)
        sub._entity_url = _entity_url(sub, cache)
        # Any user can approve/reject except their own submissions (reject allowed on own)
        submitter_id = int(sub.submitted_by_id) if sub.submitted_by_id is not None else None
        not_submitter = submitter_id != current_user.id
        sub._can_approve = not_submitter
        sub._can_reject = True
...
def reject(sub_id):
    ...
    target_id = int(sub.target_user_id) if sub.target_user_id is not None else None
    submitter_id = int(sub.submitted_by_id) if sub.submitted_by_id is not None else None
    is_submitter = submitter_id == current_user.id
    # Target user can reject rating/note submissions
    is_target = sub.type in ('rating', 'note') and target_id == current_user.id
    # Editors can reject any submission except their own
    is_editor_allowed = current_user.is_editor_or_admin and not is_submitter
    if not is_submitter and not is_target and not is_editor_allowed:
        abort(403)
